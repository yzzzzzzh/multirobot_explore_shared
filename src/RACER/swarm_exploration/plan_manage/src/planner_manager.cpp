...
bool FastPlannerManager::checkTrajCollision(double& distance, bool allow_inflation_escape) {
  ...
  bool escaping_inflation =
      allow_inflation_escape && sdf_map_->getInflateOccupancy(cur_pt) == 1 &&
      sdf_map_->getOccupancy(cur_pt) == SDFMap::FREE;
  double radius = 0.0;
  ...
  while (radius < 6.0 && t_now + fut_t < local_data_.duration_) {
    ...
    const bool inflated = sdf_map_->getInflateOccupancy(fut_pt) == 1;
    const bool known_free = sdf_map_->getOccupancy(fut_pt) == SDFMap::FREE;
    if (inflated && !(escaping_inflation && known_free)) {
      distance = radius;
      // std::cout << "collision at: " << fut_pt.transpose() << ", dist: " << dist << std::endl;
      std::cout << "collision at: " << fut_pt.transpose() << std::endl;
      return false;
    }
    if (!inflated) escaping_inflation = false;
...
bool FastPlannerManager::kinodynamicReplan(const Eigen::Vector3d& start_pt,
  ...
  if (status == KinodynamicAstar::TIMEOUT) {
    ROS_WARN("Kinodynamic search timed out; using bounded geometric fallback");
    return false;
  }
  if (status == KinodynamicAstar::NO_PATH) {
    ...
    if (status == KinodynamicAstar::NO_PATH ||
        status == KinodynamicAstar::TIMEOUT) {
      ROS_WARN_COND(
          status == KinodynamicAstar::TIMEOUT,
          "Kinodynamic retry timed out; using bounded geometric fallback");
...
void FastPlannerManager::planExploreTraj(const vector<Eigen::Vector3d>& tour,
  ...
  vector<Eigen::Vector3d> safe_tour = tour;
  if (safe_tour.empty()) {
    ROS_ERROR("Empty path to trajectory planner");
    return;
  }
  if (safe_tour.size() == 1) {
    // A viewpoint can coincide with the current pose after a frontier update.
    // PolynomialTraj::waypointsTraj requires at least two polynomial segments:
    // with only one segment its selection matrix indexes the first free
    // derivative column past the end of the matrix.  Use two equal-duration
    // constant-position segments so the current velocity can be brought to zero
    // without killing the exploration node.
    safe_tour.push_back(safe_tour.front());
    safe_tour.push_back(safe_tour.front());
    ROS_WARN("One-point exploration path; publishing a hover trajectory");
  }
...
bool FastPlannerManager::checkSwarmCollision(const int& id) {
  ...
  while (self_t < local_data_.duration_ && self_t > 0 && other_t < traj.duration_) {
    ...
    // Match the 3-D ellipsoidal separation metric used by
    // BsplineOptimizer::calcSwarmCost.  The upstream XY-only check reports a
    // collision for trajectories that are safely separated in height and
    // causes unnecessary replanning in the 3-D exploration setup.
    constexpr double vertical_scale = 2.5;
    constexpr double horizontal_scale = 1.0;
    const Eigen::Vector3d diff = self_pos - other_pos;
    const double ellip_dist =
        std::sqrt(diff.z() * diff.z() /
                      (vertical_scale * vertical_scale) +
                  diff.head<2>().squaredNorm() /
                      (horizontal_scale * horizontal_scale));
    if (ellip_dist < 0.5) return false;
