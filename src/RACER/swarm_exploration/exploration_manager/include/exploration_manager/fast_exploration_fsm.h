...
#include <exploration_manager/FrontierShare.h>
...
private:
  ...
  uint64_t nextAssignmentEpoch(const DroneState& first, const DroneState& second) const;
  void setRouteOwnership(DroneState& state, const vector<int>& ids, uint64_t epoch);
  void purgeExpiredFailures(DroneState& state, double now);
  bool isGridBlacklisted(const DroneState& state, int grid_id, double now) const;
  void reconcileDuplicateOwnership();
  bool targetHoldComplete(double now) const;
  ...
  void executionBlockedCallback(const std_msgs::BoolConstPtr& msg);
  ...
  void frontierShareTimerCallback(const ros::TimerEvent& e);
  void frontierShareMsgCallback(
  ...
  ros::Subscriber trigger_sub_, odom_sub_, execution_blocked_sub_;
  ros::Publisher replan_pub_, new_pub_, bspline_pub_;

  // Swarm state
  ros::Publisher drone_state_pub_, opt_pub_, opt_res_pub_, frontier_share_pub_,
      swarm_traj_pub_, grid_tour_pub_, hgrid_pub_;
  ros::Subscriber drone_state_sub_, opt_sub_, opt_res_sub_, frontier_share_sub_,
      swarm_traj_sub_;
  ros::Timer drone_state_timer_, opt_timer_, frontier_share_timer_, swarm_traj_timer_;
};
