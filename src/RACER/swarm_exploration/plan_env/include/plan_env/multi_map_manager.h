...
public:
  ...
  // Seal the residual address buffer before advertising peer frontier data so
  // the matching free/occupied evidence is not stranded below chunk_size_.
  void flushPendingChunk();
  bool hasMapDataFrom(const int& drone_id) const;
  int receivedChunkCount(const int& drone_id) const;
  void getChunkBoxes(
...
private:
  ...
  // When true (default upstream behaviour) a received chunk overwrites the
  // local occupancy unconditionally.  When false, remote voxels are only
  // written where this robot has no observation of its own (UNKNOWN), so a
  // misaligned partner frame can neither erase locally observed walls nor
  // paint phantom walls into locally observed free space.
  bool remote_overwrites_local_;
  int remote_skipped_known_, remote_applied_;
