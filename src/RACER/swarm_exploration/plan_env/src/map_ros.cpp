...
void MapROS::init() {
  ...
  voxel_stats_service_ = node_.advertiseService(
      "/sdf_map/voxel_stats", &MapROS::voxelStatsCallback, this);
...
bool MapROS::voxelStatsCallback(
    std_srvs::Trigger::Request& /*request*/,
    std_srvs::Trigger::Response& response) {
  uint64_t unknown = 0;
  uint64_t free = 0;
  uint64_t occupied = 0;
  const Eigen::Vector3i lower = map_->mp_->box_min_.cwiseMax(
      Eigen::Vector3i::Zero());
  const Eigen::Vector3i upper = map_->mp_->box_max_.cwiseMin(
      map_->mp_->map_voxel_num_);
  for (int x = lower.x(); x < upper.x(); ++x) {
    for (int y = lower.y(); y < upper.y(); ++y) {
      for (int z = lower.z(); z < upper.z(); ++z) {
        const int state = map_->getOccupancy(Eigen::Vector3i(x, y, z));
        if (state == SDFMap::UNKNOWN) {
          ++unknown;
        } else if (state == SDFMap::FREE) {
          ++free;
  ...
  const uint64_t total = unknown + free + occupied;
  const uint64_t observed = free + occupied;
  const double voxel_volume =
      std::pow(map_->mp_->resolution_, 3);
  const double observed_fraction =
      total == 0 ? 0.0 : static_cast<double>(observed) / total;
  ...
  json << std::fixed << std::setprecision(6)
       ...
       << ",\"total_volume_m3\":" << total * voxel_volume
       << ",\"observed_volume_m3\":" << observed * voxel_volume
       << ",\"unknown_volume_m3\":" << unknown * voxel_volume
       << ",\"resolution_m\":" << map_->mp_->resolution_ << "}";
  response.success = true;
  response.message = json.str();
  ROS_WARN("RACER_COVERAGE %s", response.message.c_str());
  return true;
}
...
void MapROS::visCallback(const ros::TimerEvent& e) {
  ...
  if (show_all_map_) {
    ...
    // Diagnostic: publish the UNKNOWN voxels (within the explored bounds)
    // once per second so frontier persistence can be inspected offline.
...
void MapROS::cloudPoseCallback(
  ...
  vector<uint8_t> endpoint_hits;
  size_t free_ray_count = 0;
  if (has_intensity) {
    pcl::PointCloud<pcl::PointXYZI> marked_cloud;
    pcl::fromROSMsg(*msg, marked_cloud);
    cloud.points.resize(marked_cloud.points.size());
    endpoint_hits.resize(marked_cloud.points.size(), 1);
    for (size_t i = 0; i < marked_cloud.points.size(); ++i) {
      cloud.points[i].x = marked_cloud.points[i].x;
      cloud.points[i].y = marked_cloud.points[i].y;
      cloud.points[i].z = marked_cloud.points[i].z;
      // The ROS2 bridge uses the lowest finite float as an explicit
      // no-return sentinel. Normal LiDAR reflectivity is nowhere near this
      // range, so existing registered hits remain backward compatible.
      if (marked_cloud.points[i].intensity < -1.0e30f) {
        endpoint_hits[i] = 0;
        ++free_ray_count;
      }
    }
    cloud.width = marked_cloud.width;
    cloud.height = marked_cloud.height;
    cloud.is_dense = marked_cloud.is_dense;
  } else {
    pcl::fromROSMsg(*msg, cloud);
  }
  int num = cloud.points.size();

  map_->inputPointCloud(
      cloud, num, camera_pos_, has_intensity ? &endpoint_hits : nullptr);
  if (free_ray_count > 0) {
    ROS_INFO_THROTTLE(5.0,
        "RACER_METRIC no_return_free_rays_fused rays=%zu total=%d",
        free_ray_count, num);
  }
