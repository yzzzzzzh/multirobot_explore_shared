...
void SDFMap::initMap(ros::NodeHandle& nh) {
  ...
  nh.param("sdf_map/euclidean_inflation", mp_->euclidean_inflation_, false);
...
void SDFMap::setCacheOccupancy(const int& adr, const int& occ) {
  // PointCloud2 is allowed to contain non-finite samples and a ray that lands
  // exactly outside the map can also produce an invalid voxel address.  The
  // upstream implementation indexed the cache vectors unconditionally, so a
  // single malformed LiDAR return killed the whole exploration node.  Keep
  // this final guard even though inputPointCloud filters endpoints below.
  const int buffer_size = static_cast<int>(md_->count_hit_.size());
  if (adr < 0 || adr >= buffer_size) {
    ROS_WARN_THROTTLE(
        1.0, "Discard out-of-map occupancy cache address %d (size=%d)", adr,
        buffer_size);
    return;
  }
...
void SDFMap::inputPointCloud(const pcl::PointCloud<pcl::PointXYZ>& points, const int& point_num,
    const Eigen::Vector3d& camera_pos, const vector<uint8_t>* endpoint_hits) {
  if (point_num <= 0 || points.points.empty()) return;
  if (!camera_pos.allFinite() || !isInMap(camera_pos)) {
    ROS_WARN_THROTTLE(
        1.0,
        "Discard point cloud with invalid/out-of-map sensor origin "
        "(%.3f, %.3f, %.3f)",
  ...
  // The ray caster walks from each return toward the sensor but deliberately
  // excludes the terminal sensor voxel.  On a moving ground robot that left
  // the current pose UNKNOWN until a later ray happened to cross the same
  // cell.  Conservative A* then had no legal first step even though the
  // physical sensor necessarily occupies free space.  Mark the sensor origin
  // as one miss for every usable scan before integrating the rays.
  Eigen::Vector3i sensor_idx;
  posToIndex(camera_pos, sensor_idx);
  ...
  int vox_adr;
  double length;
  bool any_valid_ray = false;
  const int usable_points =
      std::min(point_num, static_cast<int>(points.points.size()));
  for (int i = 0; i < usable_points; ++i) {
    ...
    const bool endpoint_is_hit =
        endpoint_hits == nullptr || i >= static_cast<int>(endpoint_hits->size()) ||
        endpoint_hits->at(i) != 0;
    int tmp_flag;
    ...
    if (!isInMap(idx)) {
      ROS_WARN_THROTTLE(
          1.0, "Discard projected LiDAR endpoint outside voxel map "
               "(%d, %d, %d)",
          idx.x(), idx.y(), idx.z());
      continue;
    }
          ...
          for (int inf_x = -inf_step; inf_x <= inf_step; ++inf_x)
            for (int inf_y = -inf_step; inf_y <= inf_step; ++inf_y)
              for (int inf_z = -inf_step; inf_z <= inf_step; ++inf_z) {
                // Upstream RACER inflates a full voxel cube.  That remains
                // the default for compatibility with the aerial setup.  A
                // planar circular robot, however, needs a Euclidean disc:
                // with 0.25 m voxels and a requested 0.60 m radius, the
                // upstream ceil-to-three-cell square reaches 1.06 m at its
                // corners and can seal otherwise traversable corridors.
                // The half-cell allowance conservatively includes voxels
                // intersected by the requested continuous-radius envelope.
                if (mp_->euclidean_inflation_) {
                  const double offset_x = inf_x * mp_->resolution_;
                  const double offset_y = inf_y * mp_->resolution_;
                  const double offset_z = inf_z * mp_->resolution_;
                  const double offset =
                      std::sqrt(offset_x * offset_x + offset_y * offset_y +
                                offset_z * offset_z);
                  if (offset >
                      mp_->obstacles_inflation_ + 0.5 * mp_->resolution_)
                    continue;
                }
