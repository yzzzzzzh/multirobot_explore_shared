...
void MultiMapManager::init() {
  ...
  node_.param("multi_map_manager/remote_overwrites_local", remote_overwrites_local_, true);
  remote_skipped_known_ = 0;
  remote_applied_ = 0;
...
void MultiMapManager::flushPendingChunk() {
  if (adr_buffer_.empty()) return;

  // A voxel may be touched by several rays before the periodic frontier share.
  // The occupancy value is queried only when the chunk is transmitted, so one
  // address is sufficient and avoids wasting bandwidth on duplicates.
  sort(adr_buffer_.begin(), adr_buffer_.end());
  adr_buffer_.erase(unique(adr_buffer_.begin(), adr_buffer_.end()), adr_buffer_.end());

  auto& own = multi_map_chunks_[drone_id_ - 1];
  MapChunk chunk;
  chunk.voxel_adrs_.swap(adr_buffer_);
  chunk.idx_ = own.chunks_.size() + 1;
  chunk.need_query_ = true;
  chunk.empty_ = false;
  own.chunks_.push_back(std::move(chunk));

  if (own.idx_list_.empty()) {
    own.idx_list_ = { static_cast<int>(own.chunks_.back().idx_),
      static_cast<int>(own.chunks_.back().idx_) };
  } else if (own.idx_list_.back() + 1 ==
             static_cast<int>(own.chunks_.back().idx_)) {
    own.idx_list_.back() = own.chunks_.back().idx_;
  } else {
    own.idx_list_.push_back(own.chunks_.back().idx_);
    own.idx_list_.push_back(own.chunks_.back().idx_);
  }
}

bool MultiMapManager::hasMapDataFrom(const int& drone_id) const {
  return drone_id > 0 && drone_id <= static_cast<int>(multi_map_chunks_.size()) &&
         !multi_map_chunks_[drone_id - 1].idx_list_.empty();
}

int MultiMapManager::receivedChunkCount(const int& drone_id) const {
  if (drone_id <= 0 || drone_id > static_cast<int>(multi_map_chunks_.size())) return 0;
  const auto& ids = multi_map_chunks_[drone_id - 1].idx_list_;
  int count = 0;
  for (int i = 0; i + 1 < static_cast<int>(ids.size()); i += 2)
    count += std::max(0, ids[i + 1] - ids[i] + 1);
  return count;
}
...
void MultiMapManager::insertChunkToMap(const MapChunk& chunk, const int& drone_id) {
  ...
  for (int i = 0; i < chunk.voxel_adrs_.size(); ++i) {
    ...
    // Local observations win: never overwrite a voxel this robot has
    // observed itself.  With a 0.5-1 m inter-robot frame error the
    // partner's free rays would otherwise erase locally observed walls
    // (doors that flicker open/closed, A* homotopy flips) and its walls
    // would block locally free corridors.
    if (!remote_overwrites_local_ && map_->getOccupancy(idx) != SDFMap::UNKNOWN) {
      ++remote_skipped_known_;
      continue;
    }
    ++remote_applied_;
  ...
  ROS_INFO_THROTTLE(10.0, "RACER_METRIC chunk_merge drone=%d applied=%d skipped_known=%d",
      drone_id_, remote_applied_, remote_skipped_known_);
}
