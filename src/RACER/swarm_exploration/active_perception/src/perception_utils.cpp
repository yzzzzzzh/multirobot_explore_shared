...
PerceptionUtils::PerceptionUtils(ros::NodeHandle& nh) {
  ...
  nh.param<std::string>("perception_utils/sensor_model", sensor_model_, "camera");
  nh.param("perception_utils/lidar_min_elevation", lidar_min_elevation_, -0.7);
  nh.param("perception_utils/lidar_max_elevation", lidar_max_elevation_, 0.9);
...
bool PerceptionUtils::insideFOV(const Vector3d& point) {
  ...
  if (sensor_model_ == "lidar_3d") {
    const double horizontal = hypot(dir.x(), dir.y());
    const double elevation = atan2(dir.z(), horizontal);
    return elevation >= lidar_min_elevation_ && elevation <= lidar_max_elevation_;
  }
...
void PerceptionUtils::getFOVBoundingBox(Vector3d& bmin, Vector3d& bmax) {
  if (sensor_model_ == "lidar_3d") {
    bmin = pos_ - Vector3d::Constant(max_dist_);
    bmax = pos_ + Vector3d::Constant(max_dist_);
    return;
  }
