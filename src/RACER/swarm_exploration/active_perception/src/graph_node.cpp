...
double ViewNode::searchPath(const Vector3d& p1, const Vector3d& p2, vector<Vector3d>& path) {
  ...
  // No path within budget: use a penalised straight-line estimate instead of
  // a constant.  A constant made every unreachable/timed-out pair cost the
  // same, so the ATSP could not distinguish a target 3 m away from one 40 m
  // away and the tour order flipped between replans.
  path = { p1, p2 };
  return 1.5 * (p1 - p2).norm() + 10.0;
}
