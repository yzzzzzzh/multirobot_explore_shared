...
public:
    ...
    /// \brief Model entity this hardware instance belongs to (sensor scoping).
...
namespace gz_quadruped_hardware
{
        ...
        int& update_rate)
    {
        ...
        // Sensors are enumerated from the whole ECM below; with several robots
        // in one world each hardware instance must only take the sensors that
        // belong to its own model.  The model entity is the parent of any of
        // the enabled joints.
        sim::Entity model_entity = sim::kNullEntity;
        if (!enableJoints.empty())
        {
            auto parent = _ecm.Component<sim::components::ParentEntity>(enableJoints.begin()->second);
            if (parent) model_entity = parent->Data();
        }
        this->dataPtr->model_entity_ = model_entity;
    ...
    void GazeboSimSystem::registerSensors(
        const hardware_interface::HardwareInfo& hardware_info)
    {
        auto belongs_to_model = [this](sim::Entity e) -> bool
        {
            const sim::Entity model = this->dataPtr->model_entity_;
            if (model == sim::kNullEntity) return true;
            while (e != sim::kNullEntity)
            {
                if (e == model) return true;
                auto parent = this->dataPtr->ecm->Component<sim::components::ParentEntity>(e);
        ...
        this->dataPtr->ecm->Each<sim::components::Imu,
            ...
            {
                if (!belongs_to_model(_entity)) return true;
        ...
        const rclcpp::Duration& /*period*/)
    {
        for (unsigned int i = 0; i < this->dataPtr->joints_.size(); ++i)
        {
            ...
            else
            {
                ...
                // gz-physics does not apply the URDF effort limit to force
                // commands; the PD above can otherwise exceed the motor's
                // peak torque several-fold when a target jumps.
                const double effort_limit = this->dataPtr->joints_[i].joint_axis.Effort();
                if (effort_limit > 0.0)
                {
                    torque = std::max(-effort_limit, std::min(effort_limit, torque));
                }
