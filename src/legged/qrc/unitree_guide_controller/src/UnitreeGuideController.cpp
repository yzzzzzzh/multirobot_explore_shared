...
namespace unitree_guide_controller
{
    ...
    update(const rclcpp::Time& time, const rclcpp::Duration& period)
    {
        ...
        ctrl_component_.wave_generator_->setTime(time.seconds());
        ...
        if (mode_ == FSMMode::NORMAL)
        {
            ...
            if ((++fsm_count % 200) == 0)
            {
                ...
                state_msg.data = current_state_->state_name_string;
                fsm_state_publisher_->publish(state_msg);
            }
            if (next_state_name_ != current_state_->state_name)
            {
                ...
                std_msgs::msg::String state_msg;
                state_msg.data = next_state_->state_name_string;
                fsm_state_publisher_->publish(state_msg);
            }
        }
        ...
        const rclcpp_lifecycle::State& /*previous_state*/)
    {
        fsm_state_publisher_ = get_node()->create_publisher<std_msgs::msg::String>(
            "fsm_state", rclcpp::QoS(rclcpp::KeepLast(1)).transient_local());
        control_input_subscription_ = get_node()->create_subscription<control_input_msgs::msg::Inputs>(
            "control_input", 10, [this](const control_input_msgs::msg::Inputs::SharedPtr msg)
            {
                ...
                if (msg->command != 0 || (++input_count % 200) == 0)
                    RCLCPP_INFO(get_node()->get_logger(), "control_input: command=%d lx=%.2f ly=%.2f rx=%.2f",
                                msg->command, msg->lx, msg->ly, msg->rx);
            });
