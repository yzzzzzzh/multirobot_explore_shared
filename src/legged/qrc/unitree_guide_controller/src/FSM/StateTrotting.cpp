...
void StateTrotting::run(const rclcpp::Time &/*time*/, const rclcpp::Duration &/*period*/) {
    {
        ...
        const bool walking = std::fabs(v_cmd_body_(0)) > 0.01 || std::fabs(v_cmd_body_(1)) > 0.01 || std::fabs(d_yaw_cmd_) > 0.01;
        if (walking) ++walk_count; else walk_count = 0;
        const bool walk_onset = walking && walk_count <= 600 && (walk_count % 20) == 0;
        if ((++diag_count % 100) == 0 || walk_onset) {
            const Vec3 pb = estimator_->getPosition();
            const Vec3 vb = estimator_->getVelocity();
            const RotMat R = estimator_->getRotation();
            const double roll = std::atan2(R(2, 1), R(2, 2));
            const double pitch = std::asin(std::max(-1.0, std::min(1.0, -R(2, 0))));
            const Vec34 feet_b = estimator_->getFeetPos2Body();
            const double q0 = ctrl_interfaces_.joint_position_state_interface_[0].get().get_value();
            const double q1 = ctrl_interfaces_.joint_position_state_interface_[1].get().get_value();
            const double q2 = ctrl_interfaces_.joint_position_state_interface_[2].get().get_value();
            std::fprintf(stderr, "[trot %p] pos=(%.2f,%.2f,%.2f) vel=(%.2f,%.2f,%.2f) pcd=(%.2f,%.2f,%.2f) rp=(%.1f,%.1f)deg contact=%d%d%d%d vcmd=%.2f qFR=(%.2f,%.2f,%.2f) footFR_b=(%.2f,%.2f,%.2f)\n",
                         static_cast<const void*>(this), pb(0), pb(1), pb(2), vb(0), vb(1), vb(2), pcd_(0), pcd_(1), pcd_(2),
                         roll * 57.3, pitch * 57.3,
                         wave_generator_->contact_(0), wave_generator_->contact_(1), wave_generator_->contact_(2), wave_generator_->contact_(3),
                         v_cmd_body_(0), q0, q1, q2, feet_b(0, 0), feet_b(1, 0), feet_b(2, 0));
            if (walk_onset) {
                const Vec34 pf = estimator_->getFeetPos();
                std::fprintf(stderr, "[walk %p] t=%d vel=(%.2f,%.2f) gyro_z=%.2f FRgoal=(%.2f,%.2f,%.2f) FRact=(%.2f,%.2f,%.2f) FLgoal=(%.2f,%.2f,%.2f) FLact=(%.2f,%.2f,%.2f) contact=%d%d%d%d\n",
                             static_cast<const void*>(this), walk_count, vb(0), vb(1), estimator_->getGyroGlobal()(2),
                             pos_feet_global_goal_(0, 0), pos_feet_global_goal_(1, 0), pos_feet_global_goal_(2, 0), pf(0, 0), pf(1, 0), pf(2, 0),
                             pos_feet_global_goal_(0, 1), pos_feet_global_goal_(1, 1), pos_feet_global_goal_(2, 1), pf(0, 1), pf(1, 1), pf(2, 1),
                             wave_generator_->contact_(0), wave_generator_->contact_(1), wave_generator_->contact_(2), wave_generator_->contact_(3));
            }
        }
    }
