...
void StateRL::run(const rclcpp::Time&/*time*/, const rclcpp::Duration&/*period*/)
{
    ...
    if (!use_rl_thread_)
    {
        // Same cadence as the thread (frequency / decimation), but in sim time.
        if (++run_counter_ >= params_.decimation)
        {
            run_counter_ = 0;
            runModel();
        }
    }
    if (++diag_counter_ >= 100)
    {
        diag_counter_ = 0;
        const auto& q = robot_state_.motor_state.q;
        const auto qc = output_dof_pos_.squeeze();
        std::fprintf(stderr,
                     "[rl %p] hips q/cmd FL %.2f/%.2f FR %.2f/%.2f RL %.2f/%.2f RR %.2f/%.2f thighFL %.2f/%.2f cmd=(%.2f,%.2f,%.2f)\n",
                     static_cast<void*>(this),
                     q[0], qc[0].item<double>(), q[3], qc[3].item<double>(),
                     q[6], qc[6].item<double>(), q[9], qc[9].item<double>(),
                     q[1], qc[1].item<double>(),
                     control_.x, control_.y, control_.yaw);
    }
    setCommand();
}
...
torch::Tensor StateRL::computeObservation()
{
    ...
    for (const std::string& observation : params_.observations)
    {
        ...
        {
            // The gyroscope already reports body-frame rates (legged_gym's
            // base_ang_vel); rotating it by the inverse base quaternion again
            // mixes roll/pitch rates as soon as the yaw departs from the IMU
            // reference heading.
